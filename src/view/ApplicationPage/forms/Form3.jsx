...
const Form3 = ({ next, prev, finance_info, banks, sendApplication }) => {
  ...
  const onFinish = async (fielValues) => {
    ...
    await sendApplication(values, "finance-info");
  ...
  return (
    ...
    /> */}
      ...
      <Form.Item
        name={["pay_date"]}
        label="What day do you get paid?"
        rules={[{ required: true }]}
      >
        <Select
          size="large"
          style={{ width: "58%" }}
          placeholder="What day do you get paid?"
        >
          {payDates.map((e, i) => (
...
export default connect(mapStateToProps, { sendApplication })(Form3);
